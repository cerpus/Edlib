...
2. Setup .env
    ```bash
    cp sourcecode/hub/.env.example sourcecode/hub/.env
    cp sourcecode/apis/contentauthor/.env.example sourcecode/apis/contentauthor/.env
    ```
...
5. Generate app key
    ...
    docker compose exec contentauthor-fpm php artisan key:generate
    ```

6. Run migration for Content author
    ```bash
    docker compose exec contentauthor-fpm php artisan migrate
    ```

...
`sourcecode/www`, and the editor is placed in `sourcecode/apis/contentauthor`.
Both have a default language of `en` and a fallback language of `en`. If a language code is not provided it will
use the default language. If there are untranslated strings or the language is not supported, the fallback language
will be used.
...
### Editor
The language files are located in `resources/lang`, and the setup of defaults are done in `config/app.php`.
To set the language use `launch_presentation_locale` in the LTI request. This should be a valid IETF BCP47 code.

When the editor is opened from the content browser, it will use the same language, if supported.
...
### Further reading
- [A more in-depth description on local codes in Content Author](./locale)  
- [A more in-depth description on translation files in Content Author](./translation-files)

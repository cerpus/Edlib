...
## H5P
Using H5P, Edlib ensures that you keep up with the leading ed-tech providers worldwide. H5P provides continuous development of new content types. At the same time, the open source community behind it helps keep the products free of bugs, translated to new languages, and generally up to date with the needs of its users.

Edlib uses the H5P Hub, which keeps the available content types found on the [h5p.org site](https://h5p.org/content-types-and-applications) up to date. Here you can view examples of each content type and try out their look and feel. These H5P libraries and many more can be found on the official [H5P GitHub account](https://github.com/h5p). 
Unofficial H5P libraries, forked or built from scratch, can also be installed and used within Edlib. For developing and installing non-official libraries, please refer to the ["Third-party content section"](https://docs.edlib.com/docs/developers/thirdPartyContent/h5p) for best practices.  

...
Since its inception, NDLA has been a pioneer regarding technology related to open educational resources. When the company that developed and created Flash-based interactive content, Amendor AS, started developing an alternative mobile-friendly content creation tool for interactives, NDLA was fast to follow. Amendor formed [Joubel](https://joubel.com/) in 2014, and the new company started the development of [H5P (HTML5 package)](https://h5p.org/content-types-and-applications) with substantial contributions from NDLA.
...
NDLA is one of Norway's pioneers regarding attribution and licensing of open educational content. Having implemented [Creative Commons](https://creativecommons.org/) as part of their vision for content creation, they also convinced H5P and Edlib to actively be part of the "commons" world. This enables NDLA to safely share their OERs which, in turn, provides their users with all the necessary license and attribution information to make safe use of them. NDLA ensures that this information is accessible to the teachers and students that need it. The attribution and license information is made available through the "Rights of use" entry on all H5P and from the Edlib APIs. All the collected data is made available &mdash;in each and every article on NDLA's site&mdash;as part of the overall instructions for reuse:
...
<div class="text--center">
    ...
    <br/>
    <em>All articles summarize the attribution and licensing information of the embedded elements - the H5P information is provided by Edlib</em>
</div> 
...
#### Version Control
Having used H5P from the very beginning, NDLA has a vast number of resources and needs to keep a tight control regarding the upgrading of library versions. This means that the existing resources in Edlib won't be affected by a system upgrade/installation of a newer version on the same library (H5P content type). Each resource can be individually upgraded to either the next available version or directly to the latest version. In addition, the admin panel enables the maintainers to do a controlled bulk upgrade of all resources from one version to another.
...
<div class="text--center">
    ...
    <br/>
    <em>Overview of installed libraries of Course Presentation with content and possibility for bulk upgrade</em>
</div> 
...
#### Enabled H5P Types
Edlib supports all H5P content types, and NDLA has chosen the varieties they want to keep in their H5P hub. This gives NDLA control over what content types are used. It also provides NDLA the possibility of doing a qualified review/test of the types they want before deciding to enable them for the organization as a whole. The review and testing are done in their test environment. It's important to note that all former H5P content types, with content, will be visible for filtering and search purposes within Edlib even if the content type has been disabled for creating new content.

### Custom H5P Content Types
NDLA has developed several H5P content types from scratch and forked existing official H5P types to add wanted functionality. These content types were made to meet the demands for creating and providing high-quality learning resources. The content types are made to work within Edlib and in general installations of H5P, for exmaple, via WordPress plugins. Each of the content types is accessible as a separate repository on NDLA's GitHub accounts, https://github.com/NDLANO and https://github.com/NDLA-H5P.   

...
H5PEditor.metadataAuthorWidget = function (semantics, params, $wrapper, parent) {
  ...
  /*
  The next functions are used to remove duplicates that exist today.
  Can be removed in the future when we believe authors are not duplicated anymore.
   */
  function exists(name, role) {
    return author => author.name === name && author.role === role;
  }

  function onlyUnique(authorsList, currentAuthor) {
    if( !authorsList.some(exists(currentAuthor.name, currentAuthor.role))){
      authorsList.push(currentAuthor);
    }
    return authorsList;
  }

  params.authors = params.authors.filter(author => author.name).reduce(onlyUnique, []);
  ...
  '</div>').children('button').click(function (event) {
    ...
    const authorDuplicate = params.authors.some(exists(name, role));
    if (authorDuplicate) {
      resetForm();
      return;
    }
  ...
  function addAuthor(name, role, readonly) {
    if( !params.authors.some(exists(name, role))){
      params.authors.push({
        name: name,
        role: role,
        readonly: !!readonly,

    ...
    self.setMetadata = function (metadata) {
      ...
      this.children
          .filter(function(element){
            return metadata.hasOwnProperty(element.field.name) && $.inArray(element.field.type, ['select', 'text', 'textarea', 'number']) !== -1;
          })
          .forEach(function (element) {
            const metadataValue = metadata[element.field.name];
            if( typeof metadataValue === 'object' && typeof metadataValue.getClassName === 'function' && metadataValue.getClassName() === 'metadata'){ //created with the function MetadataForm.createMetadataObject
